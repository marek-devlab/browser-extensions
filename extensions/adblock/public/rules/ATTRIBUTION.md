...
third-party filter-list **data** redistributed by marek-devlab. They are **not** covered by the
...
`manifest.json`). marek-devlab only removed AdGuard's metadata sentinel and `redirect` rules,
...
`cosmetic.json` is authored by marek-devlab and is covered by the extension's MIT license.

Full notices for everything the marek-devlab extensions redistribute: `THIRD-PARTY-NOTICES.md`
in the source repository. Contact: marek.devlab@gmail.com
